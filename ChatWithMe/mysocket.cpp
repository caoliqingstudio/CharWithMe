...
MySocket::MySocket(QString ip, QString port, QTcpSocket *insocket){
    myconnect=insocket;
    this->ip=ip;
    this->port=port;
}
...
int MySocket::sendQuit(QString username){
    QString string=QString::number(LISTENING)+"\n"+username+"\n";
    //qDebug()<<"\n\n\n\nsend quit "<<string;
    (*myconnect).write(string.toStdString().c_str());
