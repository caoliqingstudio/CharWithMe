    ...
    if(sendtype){//发文件
        ...
        if(!sendsocket.waitForConnected(3000)){
            ...
            *filename=fileList.at(fileList.count()-1);
            ...
            qDebug()<<"have send start information";
//            sendsocket.disconnectFromHost();
//            sendsocket.waitForDisconnected();
//            sendsocket.connectToHost(address,FILE_PORT);
//            if(!sendsocket.waitForConnected()){
//                QMessageBox::warning(0,"SendFile","fail to send;please check the user Net and PC!");
//                readfile.close();
//                //this->~Thread();//this->deleteLater();
//                this->quit();return;
//            }
...
//                testfile.write(text,couts*8+16);
                do{
                    sendsocket.write(QByteArray::fromRawData(text,couts*8+16));
                    sendsocket.waitForBytesWritten();
...
//            QMessageBox::warning(0,"ReceiveFile","fail to receive;please check net or PC!");
//            //this->~Thread();//this->deleteLater();
//            this->quit();return;
//        }
//        mysocket=server.nextPendingConnection();
        ...
        while(mysocket->waitForReadyRead()){
            ...
            QByteArray barry=mysocket->read(blockNum);
            int num=blockNum/8-1;
            char *receivetext=barry.data();
            for(int i=0;i<num;i++){
                temp=temp^*(((quint64*)receivetext)+i);
                (*(((quint64*)receivetext)+i))=myspn.decrypt(*(((quint64*)receivetext)+i));
            }
            temp=temp^*(((unsigned long long*)receivetext)+num);
            (*(((unsigned long long*)receivetext)+num))=myspn.decrypt(*(((unsigned long long*)receivetext)+num));
            int j=7;
            while(receivetext[num*8+j]==0){
                j--;
                barry.chop(1);
                blockNum--;
            }
            ...
            int filenum=receivefile.write(barry);
            qDebug()<<"this is "<<first<<" file "<<filenum;
            tempend=tempend^temp;
            first++;
